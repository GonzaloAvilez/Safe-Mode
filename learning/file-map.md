...
- `src/lib/entries.ts` — `submitEntry`: the full moderate → embed → match → store
  ...
  writes `outcome` (known at insert time for crisis/general_flagged/cap_reached, `null`
  for the "proceed" path), and a new `updateEntryOutcome` backfills `matched`/`no_match`
...
- Unit test files live alongside their source (not inventoried individually here) — 20
  files, 137 tests, all passing as of 2026-07-25. `src/lib/entries.test.ts` — **known**,
  edited directly again for Section 4 Task 2: added `updateMock`/`eqMock` to the mock
  chain (the file never mocked `.update()` before, only `.insert()`), and `outcome` to
  the existing `insertMock` assertions. Several real typos along the way (`updatedMock`
  vs `updateMock`, then `updateMockl`) that broke the suite entirely until caught reading
  the actual error output — see [[sql-update-without-where-is-dangerous]] for the more
  significant bug from this same task. `src/lib/phrases.test.ts`
