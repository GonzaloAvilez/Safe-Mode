...
// Original Spanish source recovered from commit 21f0bcf, before the English
// transcreation. These are intentionally not translated back from English.
const spanishSeedPhrases: SeedPhrase[] = [
  // Loneliness / feeling misunderstood
...
const corpusByLocale: Record<SeedLocale, SeedPhrase[]> = {
  en: englishSeedPhrases,
  es: spanishSeedPhrases,
};
...
async function main() {
  ...
  // Dry runs do not initialize OpenAI at all, so inventory remains free and works
  // without an API key. These imports happen only after the explicit --write gate.
  const [{ getEmbedding }, { recordEmbeddingSpend }] = await Promise.all([
    import("@/lib/openai"),
    import("@/lib/spend"),
  ]);
  ...
  for (const [index, phrase] of plan.missing.entries()) {
    const position = `[${index + 1}/${plan.missing.length}]`;
  ...
  console.log(`\nDone. ${inserted} inserted, ${plan.existing.length} skipped, ${failed} failed.`);
