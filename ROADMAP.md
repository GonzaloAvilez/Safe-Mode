...
- [ ] **Human pre-approval before a phrase publishes** — raised 2026-07-15, not yet built. Today, `finalizeUserPhraseModeration` activates a phrase automatically the moment OpenAI's moderation verdict comes back clean — no human sees it first; `/admin/phrases` is an audit layer, not a gate (`approvePhrase`/`rejectPhrase`/`setPhraseActive` already exist and could back this). Open question to resolve before building: does the gate apply to Leave a Trace's public submissions only, both Leave a Trace and Contribute, or Contribute only — Leave a Trace is arguably the higher-risk source (anonymous public) despite Contribute being the one that prompted the conversation.
- [ ] **Multi-language content / segmented matching** — raised 2026-07-15 alongside the language decision above, not yet built. Decision so far: segment rather than mix — a submitted phrase's language would be tagged and matching (`match_phrase`, Observe's similarity threshold) would stay scoped to same-language pairs, since `text-embedding-3-small`'s cross-lingual similarity is measurably weaker than same-language and the current 0.5 threshold is calibrated only on the Spanish corpus. No language-detection, no `language` column, no matching changes built yet.
