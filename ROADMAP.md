...
- [x] **D13 Mirror screen** — its own screen (`/mirror`): violet "other" node, quote reveals letter-by-letter, "this resonated with me" toggle. Reached on both `matched` and `no_match`.
...
- [x] **Searching, Mirror, Gratitude — three more screens, discovered 2026-07-12, built 2026-07-13.** Real mockups (`refugio-all-screens.html`, `bocetos_pantallas_05_06_08.html`, `refugio_spec_design_v3.png`) revealed the flow is actually **8 screens**, not 5: Arrive → Observe → Remember → Write → **Searching** → **Mirror** → **Gratitude** → Leave a Trace.
  ...
  - **Mirror** (`/mirror`) — violet "other" node, quote reveals letter-by-letter, "this resonated with me" toggle (persists to `responses.wants_reply`). Reached on `no_match` too, not just `matched` — the node dims rather than the visitor dead-ending in Write. Follow-up adjustment pass resolved 2026-07-27: matched-branch tagline/subcopy rewritten to stop presuming the visitor's own feelings, matching the design brief's evidence-not-persuasion principle.
...
- [x] **D16 Fake door — reply** — covered, not exactly by original design. Mirror's "this resonated with me" toggle writes to `responses.wants_reply`, but signals "this resonated with me" rather than the "I want to reply" intent D16 originally specced. Treated as done; revisit only if the distinction turns out to matter for D27-28 metrics.
  **Confirmed 2026-07-27:** `wants_reply` is write-only today — nothing reads it anywhere (`grep` across `src/`, no admin view, no dashboard). Decision: keep the write path and the Mirror toggle (near-zero cost, and the toggle gives the visitor a real interaction moment independent of whether anyone ever looks at the data) — same reasoning as the D27-28 completion-rate descope, no dashboard is worth building for a ~10-user MVP; query `responses.wants_reply` directly by SQL if the number is ever needed for demo day or interviews.
